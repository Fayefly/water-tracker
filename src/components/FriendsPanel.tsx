      ...
      {friends.length > 0 && !expanded && (
        <div className="mt-3 space-y-1.5">
          {friends.map((friend) => {
            ...
            return (
              <div key={friend.uid} className="flex items-center gap-3 px-1">
                <span className="text-sm text-gray-600 w-20 shrink-0 truncate">{friend.userName}</span>
                ...
                <span className="text-xs text-gray-400 w-14 shrink-0 text-right">{total}ml</span>
              </div>
            );
          })}
        </div>
